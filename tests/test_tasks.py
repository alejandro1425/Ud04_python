...
def test_dashboard_requires_login(client):
    ...
    assert b"Para continuar inicia sesi\xc3\xb3n" in response.data
...
def test_create_task(auth, client, app):
    ...
    assert b"Tarea creada" in response.data
...
def test_update_task(auth, client):
    ...
    assert b"Tarea actualizada" in response.data
    assert b"Actualizada" in client.get("/tareas/", follow_redirects=True).data
...
def test_toggle_complete(auth, client):
    ...
    assert b"marcada como completada" in response.data

    response = client.post("/tareas/1/completar", follow_redirects=True)
    assert b"marcada como pendiente" in response.data
...
def test_delete_task(auth, client):
    ...
    assert b"Primera tarea" not in client.get("/tareas/", follow_redirects=True).data
