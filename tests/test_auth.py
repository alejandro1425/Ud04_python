...
def test_login_logout_cycle(client):
    ...
    assert "Sesión iniciada" in login_response.get_data(as_text=True)

    logout_response = client.get("/auth/logout", follow_redirects=True)
    assert "Sesión cerrada" in logout_response.get_data(as_text=True)
...
def test_login_invalid_credentials(client):
    ...
    assert "Contraseña incorrecta" in response.get_data(as_text=True)
